...
- Mutations check the exact configured Origin. Private mutations additionally require the session’s CSRF token. Production cookies use the Firebase-compatible name `__session`, with HttpOnly, SameSite=Lax, Secure, Path=/ and no Domain attribute. Firebase Hosting forwards only this specially named cookie to Cloud Run. All API responses, including errors, use `Cache-Control: no-store`. See [Firebase's cookie forwarding documentation](https://firebase.google.com/docs/hosting/manage-cache#using_cookies).
...
- `DELETE /api/projects/:id` accepts `{confirmation: "exact project name"}` and requires owner authentication plus CSRF. It removes only that project and revokes all its capability links. Export first to retain its history. This remains available at project/history limits so a full workspace can recover capacity without deleting the account.
...
Each workspace document is limited to 720 KB, including conservative headroom reserved for live client decisions. Each HTTP JSON body is limited to 100 KB, and projects to 30 requests / 80 deliverables / 50 baseline snapshots / 100 audit events. Initial projects allow at most 40 deliverables and 5,000 hours. Plans allow 3/15/60 stored projects and 30/300/1,500 analyses per UTC calendar month. Archive retains stored-project usage. Export and delete a completed project to free capacity. Reaching a limit returns an explicit error instead of truncating history. Unchanged draft saves and repeat sharing do not consume extra audit entries. Large accounts require future migration to separate project/event collections.
